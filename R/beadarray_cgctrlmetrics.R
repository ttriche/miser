...
bactrl = function(baseline = 3000, biotin.baseline = 1, rs = rs, gs = gs, cdf = cdf){
  # bactrl
  # Calculate quality metrics from control probes per BeadArray guidelines. 
  # 
  # Notes:
  ...
  # 
  # Arguments:
  # baseline (int): Baseline offset for Extension green signal based background (see source 2)
  # biotin.baseline (int): Special baseline offset for biotin staining background probes.
  # rs (data.frame): Red signal data (cols = probes, rows = samples, colnames = addresses).
  # gs (data.frame): Grn signal data (cols = probes, rows = samples, colnames = addresses).
  # cdf (data.frame): Control probe annotations (cols = properties, rows = probes).
  # 
  # Returns:
  # rm (results matrix): Control probe metrics (cols = metrics, rows = samples).
